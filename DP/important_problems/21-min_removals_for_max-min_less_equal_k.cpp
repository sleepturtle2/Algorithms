...
int findInd_bin_search(int key, int i,
                       int n, int k, int arr[])
{
...
// to be removed
int removals_bin_search(int arr[], int n, int k)
{
  int i, j, ans = n - 1;
  ...
  for (i = 0; i < n; i++)
  {
    ...
    // Find j such that
    // arr[j] - arr[i] <= k
    j = findInd_bin_search(arr[i], i, n, k, arr);
...
int removals_dp(int arr[], int n, int k)
{
...
int main()
{
  ...
  int k = 4;
  cout << removals(a, n, k) << endl;            //o(n2), o(n2)
  cout << removals_bin_search(a, n, k) << endl; //o(nlogn), o(n)
  cout << removals_dp(a, n, k) << endl;         //o(nlogn) , o(n)
  return 0;
}
